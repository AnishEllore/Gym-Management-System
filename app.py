...
def memberDash(username):
	if username!=session['username'] and session['prof']==4:
		flash('You aren\'t authorised to look into other dashboards', 'warning')
		return redirect(url_for('memberDash', username = session['username']))
	cur = mysql.connection.cursor()
	...
	if total==0:
		flash('The user has no progress yet', 'info')
		return render_template('home.html')
